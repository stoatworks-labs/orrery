# Running unsigned Orrery builds
...
```sh
xattr -dr com.apple.quarantine ~/Documents/"Resolume Arena"/"Extra Effects"/Orrery.bundle
```
...
```sh
codesign --force --deep --sign - ~/Documents/"Resolume Arena"/"Extra Effects"/Orrery.bundle
```

Verify it took:

```sh
codesign -dv --verbose=4 ~/Documents/"Resolume Arena"/"Extra Effects"/Orrery.bundle
spctl -a -vv ~/Documents/"Resolume Arena"/"Extra Effects"/Orrery.bundle   # still reports "rejected" — ad-hoc is not notarization
```
